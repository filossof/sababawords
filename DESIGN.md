...
## Sounds
All synthesized with the Web Audio API (`src/sound.ts`, no audio files; unlocked on pointerdown/touchend/click for iOS): tap, key, correct chime, wrong buzz,
...
## Test input is English-only, translated automatically
The user types/pastes English words (one per line or comma separated). `src/engine/translate.ts` translates
to Hebrew in the browser: Google's public web endpoint first (accurate, with alternatives; unofficial, rate-limited),
